package com.spark.coinpaypddd.signup;
...
import com.spark.coinpaypddd.acceptances.process.AcceptancesProcessTextActivity;
...
public class SignUpActivity extends BaseActivity implements SignUpContract.SignView {
    ...
    private GetImgDialog mGetImgDialog;
    private String InvateCodeMust = "币承兑BTDSubverter币赚币客汇蜂";
    boolean forceRecommendRegister = false;//是否强制输入邀请码
    ...
    protected void initData() {
        ...
        if (entity != null) {
            forceRecommendRegister = entity.isForceRecommendRegister();
        }
        if (forceRecommendRegister) {
            etPromoCode.setHint(R.string.str_promotion_code_must);
        } else {
            etPromoCode.setHint(R.string.str_promotion_code);
        }

        //希锦家需要输入邀请码
        String appName = getResources().getString(R.string.app_name);
        if (InvateCodeMust.contains(appName)) {
            etPromoCode.setHint(R.string.str_promotion_code_must);
        } else {
            etPromoCode.setHint(R.string.str_promotion_code);
        }
    ...
    protected void checkInput() {
        ...
        if (StringUtils.isEmpty(phone) && !isEmail) {
            ...
            ToastUtils.showLong(activity, getString(R.string.str_pwd_register_tag));
        } else {
            if (StringUtils.isNotEmpty(inviteCode)) {
...
//                presenter.captch();
    ...
    public void checkInviteCodeFail(HttpErrorEntity httpErrorEntity) {
        if (httpErrorEntity != null) {
            if (httpErrorEntity.getCode() == 40003) {
                ToastUtils.showToast("邀请码不存在");
            } else {
                ToastUtils.showToast("校验邀请码失败！");
            }
        }
    }
    ...
    public void dealError(HttpErrorEntity httpErrorEntity) {
        ...
        if (code == CAPTCH && StringUtils.isNotEmpty(httpErrorEntity.getData()) && httpErrorEntity.getData().contains("img")) {
        ...
        } else if (code == CAPTCH && StringUtils.isNotEmpty(httpErrorEntity.getData()) && httpErrorEntity.getData().contains("gee")) {
            cid = httpErrorEntity.getCid();
            gt3GeetestUtils = new GT3GeetestUtilsBind(activity);
            presenter.captch();
            ...
            cid = httpErrorEntity.getCid();
            handler.sendEmptyMessage(1);
        } else if (code == CAPTCH2 && StringUtils.isNotEmpty(httpErrorEntity.getData()) && httpErrorEntity.getData().contains("gee")) {
            ToastUtils.showToast(getResources().getString(R.string.str_code_error));
            cid = httpErrorEntity.getCid();
            gt3GeetestUtils = new GT3GeetestUtilsBind(activity);
            presenter.captch();
        } else if (code == CAPTCH2 && StringUtils.isNotEmpty(httpErrorEntity.getData()) && httpErrorEntity.getData().contains("phone")) {
            ToastUtils.showToast(getResources().getString(R.string.str_code_error));
        } else {
     ...
     * 展示发送短信验证码弹框
     */
    private void showImgVertifyDialog(Bitmap bitmap) {
        if (mGetImgDialog == null) {
            mGetImgDialog = new GetImgDialog(this);
            mGetImgDialog.setClickListener(new GetImgDialog.ClickLister() {
        ...
        mGetImgDialog.setBitmap(bitmap);
        mGetImgDialog.show();
    }
