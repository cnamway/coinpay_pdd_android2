...
public class BindPayPalActivity extends BaseActivity implements BindAccountContract.PaypalView {
    ...
    private void confirm() {
        ...
        if (StringUtils.isEmpty(account, pwd, dayLimit, name)) {
            ToastUtils.showToast(this, R.string.incomplete_information);
        } else {
            if (isUpdate) {
                presenter.doUpdatePaypal(payWaySetting.getId(), GlobalConstant.PAYPAL, account, getString(R.string.str_paypal), "", pwd, warn, dayLimit, name, "");
            } else {
                presenter.doBindPaypal(GlobalConstant.PAYPAL, account, getString(R.string.str_paypal), "", pwd, warn, dayLimit, name, "");
            }
