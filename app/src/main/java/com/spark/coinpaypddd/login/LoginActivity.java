...
import com.spark.coinpaypddd.my.forgot_pwd.ForgotPwdActivity;
import com.spark.coinpaypddd.signup.SignUpActivity;
...
//            String appName = "搬搬赚";
//            String apiAddress = "newbtc.online";

            String appName = "汇蜂";
            String apiAddress = "oznex.com";

//            String appName = "几何维度";
//            String apiAddress = "999fit.com";
    ...
    protected void checkInput() {
        ...
        if (StringUtils.isEmpty(username)) {
            ...
            presenter.casLogin(username, password, "true");
        }
    }
    ...
    public void casLoginSuccess(Object o) {
        gtc = (String) o;
        LogUtils.e("登录成功LoginActivity===========gtc==" + gtc);
        doUcLogin();
        isCasLogin = true;
        //保存手机号码
        String username = StringUtils.getText(etUsername);
    ...
    public void ucLoginSuccess(String response) {
        ...
        if (ucLoginSuccess && acLoginSuccess && otcLoginSuccess) {
            isCasLogin = false;
            saveOtcSid();
...
//                presenter.getPhoneCode(strAreaCode + username);
                    presenter.getPhoneCode(username);
                }
     ...
     * @return
     */
    private boolean isApiAddressSeted() {
        String apiAddress = SharedPreferencesUtil.getInstance(activity).getStringParam(SP_KEY_API_ADDRESS);
        if (StringUtils.isEmpty(apiAddress) || StringUtils.isEmpty(UcUrls.getInstance().getHost())) {
