...
import com.spark.coinpaypddd.adapter.MyOrderIngAdapter;
...
import com.spark.coinpaypddd.event.TimeDownFinishEvent;
import com.spark.coinpaypddd.my.order.detail.MyOrderDetailIngActivity;
...
public class OrderIngFragment extends BaseFragment implements MyOrderContract.MyOrderView {
    ...
    public static OrderInTransitVo orderInTransit;
    ...
    public void onEvent(TimeDownFinishEvent event) {
        if (adapter != null) {
            if (event.randomTime < 0) {//倒计时为负数时，每隔10秒刷新一次数据
                ...
                handler_timeCurrent.sendEmptyMessageDelayed(1, tick * 1000);
            } else {
                startFlush();
            }
        }
