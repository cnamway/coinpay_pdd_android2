...
import com.spark.coinpaypddd.view.ConfirmPayDialog;
import com.spark.coinpaypddd.view.PayCodeDialog;
...
public class PayActivity extends BaseActivity implements PayContract.PayView {
    ...
    @BindView(R.id.llBankQrcode)
    LinearLayout llBankQrcode;
    @BindView(R.id.ivRecCodeBank)
    ImageView ivRecCodeBank;
        ...
        if (bundle != null) {
            ...
            switch (type) {//1支付宝、2微信、3银行卡、4PAYPAL
                ...
                case 3://3银行卡
                    ...
                    if (StringUtils.isNotEmpty(curPayData.getQrCodeUrl())) {
                        llBankQrcode.setVisibility(View.VISIBLE);
                    } else {
                        llBankQrcode.setVisibility(View.GONE);
                    }
                    break;
                case 4://4PAYPAL
    ...
    @OnClick({R.id.tvPayFinish, R.id.ivRecCode, R.id.tvRecCopy, R.id.tvRecAccountCopy, R.id.tvPayReferCopy,
            R.id.tvPayPerCopy, R.id.tvOpenBankCopy, R.id.tvBranchCopy, R.id.tvBankNumCopy, R.id.tvPayNumCopy, R.id.tvJump, R.id.ivRecCodeBank})
    @Override
