...
public class GuardService extends Service {
    ...
    //WebSocket地址配置
//    private static String WEBSOCKET_HOST_AND_PORT = "ws://ws.bitaccept.oznex.com/ws";//通用
//    private static String WEBSOCKET_HOST_AND_PORT = "ws://ws.bizhuanex.com/ws";//币赚
//    private static String WEBSOCKET_HOST_AND_PORT = "ws://ws.bikeshangmao.com/ws";//币客
    private static String WEBSOCKET_HOST_AND_PORT = "";

    //
    public static void setHost(String host) {
        if (host.contains("bench")) {
            WEBSOCKET_HOST_AND_PORT = "ws://192.168.2.183:38985/ws";
        } else if (host.contains("bizhuanex")) {
            WEBSOCKET_HOST_AND_PORT = "ws://ws.bizhuanex.com/ws";
        } else if (host.contains("bikeshangmao")) {
            WEBSOCKET_HOST_AND_PORT = "ws://ws.bikeshangmao.com/ws";
        } else {
            WEBSOCKET_HOST_AND_PORT = "ws://ws.bitaccept." + host + "/ws";
        }
    }
    ...
    private synchronized void initWebSocketConnect() {
        if (isClosed && isNeedReconnect) {
            if (checkNetwork(this)) {
                Log.e(TAG, "GuardService- 创建一个新的连接WEBSOCKET_HOST_AND_PORT==" + WEBSOCKET_HOST_AND_PORT);
                new InitSocketThread().start();//创建一个新的连接
            } else {
                Log.e(TAG, "GuardService- 当前无网络连接，10秒后重新检测网络，并尝试重新连接websocket");
                mTimeHandler.sendEmptyMessageDelayed(1, CONNECT_AGAIN * 1000);
            }
        }
    }
