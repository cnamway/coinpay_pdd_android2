...
public class MathUtils {
     ...
     * @return
     */
    public static String getRundNumberForShow(double number, int n, String pattern) {
        String text = Double.toString(Math.abs(number));
        int integerPlaces = text.indexOf('.');
        int decimalPlaces = text.length() - integerPlaces - 1;
        if (decimalPlaces > n) {
     ...
     * @return
     */
    public static String getRundNumber2(String number, int n, String pattern) {
        String text = number;
        int integerPlaces = text.indexOf('.');
        int decimalPlaces = text.length() - integerPlaces - 1;
        if (decimalPlaces > n) {
            ...
            pattern += str;
            int m = (int) Math.pow(10, n);
            double result = (Math.floor(Double.valueOf(number) * m)) / (m * 1.0);
            return new DecimalFormat(pattern).format(result);
        } else {
            return number + "";
        }
    }
