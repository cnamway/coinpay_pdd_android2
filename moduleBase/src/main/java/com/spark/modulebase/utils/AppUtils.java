...
public class AppUtils {
     ...
     * @param drawableResId
     */
    public static void scaleImage2(final Activity activity, final View view, int drawableResId) {
        // 获取屏幕的高宽
        ...
        view.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                if (scaledBitmap.isRecycled())
                    ...
                    return true;
                // 当UI绘制完毕，我们对图片进行处理
                int vieWidth = view.getMeasuredWidth();
                // 计算将要裁剪的图片的顶部以及底部的偏移量
                int offset = (scaledBitmap.getWidth() - vieWidth) / 2;
                //优化小米系列手机在缩放时offset小于0的问题
                if (offset < 0) {
                    offset = 0;
                }
                ...
                finallyBitmap = Bitmap.createBitmap(scaledBitmap, offset, 0, width, hight);
                if (!finallyBitmap.equals(scaledBitmap)) {//如果返回的不是原图，则对原图进行回收
                    scaledBitmap.recycle();
                    System.gc();
                }
