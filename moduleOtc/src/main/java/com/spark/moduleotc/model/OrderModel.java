...
import com.spark.library.acp.model.MessageResultPageOrderAchiveVo;
...
public class OrderModel {
     ...
     * 查询在途订单
     */
    public void findOrderInTransit(final ResponseCallBack.SuccessListener<MessageResultPageOrderInTransitVo> successListener, final ResponseCallBack.ErrorListener errorListener) {
        ...
        queryParamOrderInTransit.setPageSize(10);
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                orderApi.findOrderInTransitUsingPOST(queryParamOrderInTransit, new Response.Listener<MessageResultPageOrderInTransitVo>() {
                    @Override
                    public void onResponse(MessageResultPageOrderInTransitVo response) {
                        int code = response.getCode();
                        if (code == SUCCESS_CODE) {
                            if (successListener != null) {
                                successListener.onResponse(response);
     ...
     * 查询进行中订单、超时单
     */
    public void findOrderIng(HashMap<String, String> params, final ResponseCallBack.SuccessListener<List<OrderInTransitVo>> successListener,
                             final ResponseCallBack.ErrorListener errorListener) {
        final QueryParamOrderInTransit queryParamOrderInTransit = new QueryParamOrderInTransit();
     ...
     * 查询已完成订单、已取消订单
     */
    public void findOrderFinish(HashMap<String, String> params, final ResponseCallBack.SuccessListener<List<OrderAchiveVo>> successListener,
                                final ResponseCallBack.ErrorListener errorListener) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                orderApi.findOrderOverUsingPOST(queryParamOrderInTransit, new Response.Listener<MessageResultPageOrderAchiveVo>() {
                    @Override
                    public void onResponse(MessageResultPageOrderAchiveVo response) {
                        int code = response.getCode();
                        if (code == SUCCESS_CODE) {
                            if (successListener != null) {
                                successListener.onResponse(response.getData().getRecords());
